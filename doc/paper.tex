...
\begin{itemize}
  ...
  \item applications
        ...
        \end{itemize}
  \item lifted inference in other domains
        \begin{itemize}
...
\todo[inline,caption={}]{
  ...
  \begin{itemize}
    ...
    \item When checking whether recursion is possible between a source and a
          ...
          seen as transitory, and allowing such recursive calls leads to FCGs
...
\todo[inline]{Explain arity and its notation. Define `ground'?}
...
variable or a constant. Throughout the paper, we use set-theoretic notation,
interpreting a formula as a set of clauses and a clause as a set of literals.

For any clause or literal $l$, let $\Vars(l)$ denote the set of variables in
$l$. For any (bound) variable $v$, let $\Dom(v)$ denote the domain over which
$v$ is quantified. For a literal $l$,
...
\begin{definition}[Model]\label{def:model}
  ...
  \end{itemize}
  Then an \emph{interpretation} of $\phi$ (with respect to $\sigma$) is a set
  $M$ of ground literals defined by adding either $p(\mathbf{t})$ or
  $\neg p(\mathbf{t})$ for every predicate $p/n$ in $\phi$ and $n$-tuple
  $\mathbf{t} \in \prod_{i=1}^{n} \sigma(\Delta_{i}^{p})$. An interpretation is
  a \emph{model} if it satisfies $\phi$ (see Appendix~A of~Dilkas and
...
\begin{algorithm}[t]
  \caption{The main part of \Cranetwo{} that compiles a formula into a set of
    equations, including the base cases of recursive functions}\label{alg:main}
  \KwIn{formula $\phi$}
...
\todo[inline,caption={}]{
  \begin{itemize}
    ...
    \item \textsc{Crane}, \Cranetwo{}, or \Crane (here and elsewhere)?
  \end{itemize}
}
...
\todo[inline]{Insert motivation for smoothing from Section 3.4.\ of the ForcLift
  paper.}
...
in an infinite loop whenever a cycle is encountered. \Cref{alg:smoothing}
...
\begin{enumerate}
  \item Whenever the set of unit clauses of the child node contains two formulas
        $\phi(x)$ and $\forall X \in \Delta\text{.
        } X \ne x \Rightarrow \phi(X)$ (i.e., the only difference between the
        two formulas is that one has the constant $x$ whereas the other one has
        a variable $X \ne x$), merge them into $\forall X \in \Delta\text{.
        }\phi(X)$.
  \item Add smoothing nodes below the domain recursion node for the difference
        ...
        step above. For example, if the child node `covers' only $p(x)$, then
