    ...
    \item Add some papers mentioned in:
          \begin{itemize}
            ...
            \item my previous paper, including:
                  ...
                  \end{itemize}
          \end{itemize}
    \item introduce \Cranetwo{}
    \item (maybe) introduce lifted/liftable
  \end{itemize}
}
...
\todo[inline]{There are three subsections now, not two.}

\subsection{First-Order Logic and WFOMC (TODO)}\label{sec:logic}
...
\begin{definition}[WFOMC]
  ...
  \emph{weights}. Unless specified otherwise, we assume weights to be equal to
  one. The \emph{(symmetric) weighted first-order model count} (WFOMC) of $\phi$
  (with respect to $\sigma$, $w^{+}$, and $w^{-}$) is the quantity
  \[
...
clauses with more than two variables, and the equality predicate.

\begin{remark}
  In the case of \textsc{ForcLift} and its extensions, support for a formula as
  valid input does not imply that the algorithm will be able to compile the
  formula into a circuit or graph suitable for lifted model counting. However,
  it is known that \textsc{ForcLift} compilation is guaranteed to succeed on any
  \FO{} formula without constants and with at most two
...
models to have \emph{exactly three positive literals with predicate $P$}. We
further illustrate the differences among the three logics with the example
below.

\begin{example}[Functions]\label{example:functions}
  To define predicate $P$ to be an endofunction on $\Delta$, in \Ctwo{} one
  could write $\forall x \in \Delta\text{. }\exists^{=1} y \in \Delta\text{.
  ...
  \begin{equation}\label[formula]{eq:functions1}
    ...
    \end{gathered}
  \end{equation}
  Although \cref{eq:functions1} has some additional models, the negative weight
  $w^{-}(S) = -1$ makes some of the counts cancel out so that the WFOMC of
  \cref{eq:functions1} is still equal to the number of endofunctions on
  $\Delta$.

  In \FO{}, we would write
...
et al.~\shortcite{DBLP:conf/ijcai/BroeckTMDR11} generalise smoothness to
...
et al.~\shortcite{DBLP:conf/ijcai/BroeckTMDR11} even further.
