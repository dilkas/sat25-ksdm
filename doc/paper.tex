...
beyond the reach of \textsc{ForcLift}, was incomplete in that it could only
construct functions definitions, which would then need to be evaluated to
...
a WFOMC problem are then compiled to C++ programs.\footnote{Compilation to C++
  has been explored before~\cite{DBLP:conf/kr/KazemiP16}, although with limited
  capabilities in terms of what formulas can be handled in a lifted manner.}
Finally, \cref{sec:experiments} reports on two experiments that compare
\Cranetwo{} with other state-of-the-art WFOMC algorithms.
...
previously considered by Dilkas and Belle~\shortcite{DBLP:conf/kr/DilkasB23}:
...
\begin{enumerate*}[label=(\roman*)]
  ...
  introduced by Dilkas and Belle~\shortcite{DBLP:conf/kr/DilkasB23}.
