...
\todo[inline,caption={}]{
  \begin{itemize}
    \item say something about what $\expr{}$ can contain (function calls,
          ...
          summations)
    \item The definition of a function call contradicts the base case finding
          algorithm. Maybe rephrase it to only look at function calls from $f$
          to itself and assume:
          \begin{itemize}
            \item no mutual recursion,
            \item these calls in particular have this form
          \end{itemize}
  \end{itemize}
}
...
\begin{example}\label{example:overall}
  ...
  Note that these base cases overlap when $l = m = 0$ but are consistent with
  each other since $0^{0} = 1$. More generally, let $\phi$ be a formula with two
  domains $\Gamma$ and $\Delta$, and let $n, m \in \mathbb{N}_{0}$. Then the
  model count of \Propagate{$\phi$, $\Delta$, $n$} assuming $|\Gamma| = m$ is
  the same as the model count of \Propagate{$\phi$, $\Gamma$, $m$} assuming
  $|\Delta| = n$.
\end{example}

\todo[inline]{This last claim sounds like something that should be proven.}
