...
\todo[inline,caption={}]{
  \begin{itemize}
    ...
    \item introduce the Iverson bracket (somewhere)
    \item reorder the Logic subsection to cover \FO{} before comparing it to
          other logics (and clarify the differences between the input format and
          the internal format)
    \item (somewhere) mention the trick about increasing the number of sorts
  \end{itemize}
}
...
\end{definition}
\todo[inline]{do I ever use this notation for the case when some of the
...
\begin{definition}
  ...
  signature, and $\expr{}$ is an algebraic expression. Henceforth, we call
  $f(\mathbf{x})$ and $\expr{}$ the left-hand side (LHS) and the right-hand side
  (RHS) of the equation, respectively.
\end{definition}
...
\todo[inline,caption={}]{
  ...
  \begin{itemize}
    \item introduces a new variable,
    \item calls itself.
  \end{itemize}
}
    ...
    \ForEach{function call $f(\mathbf{x} - \mathbf{c}) \in \expr{}$}{\label{line:functioncall}
      \ForEach{$c_{i} \in \mathbf{c}$}{
        \For{$n \gets 0$ \KwTo $c_{i} - 1$}{\label{line:lim}
          $\mathcal{B} \gets \mathcal{B} \cup \{\, f(\mathbf{x})[x_{i} \mapsto n] \,\}$\;\label{line:insert}
        }
      }
    }
  }
...
\begin{algorithm}[t]
  ...
  \Else{
    $c \gets$ a new constant in $\Delta$\;
    \ForEach{clause $C \in \phi$}{
      $C' \gets C$\;
