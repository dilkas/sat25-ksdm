...
\newtheorem{fact}{Fact}
% TODO: fix styling, check if everything in the header is still needed
% TODO: make sure forest arrows match non-forest arrows
...
\todo[inline,caption={}]{
  \begin{itemize}
    ...
    \item Can I prove that Crane's domain recursion can always simulate
          ForcLift's domain recursion?
  \end{itemize}
}
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}
    ...
    \draw[-Latex] (alg1) -- node[above] {$E$} (simplification);
    \draw[-Latex] (simplification) -- node[above] {$E'$} (compilation);
...
\begin{theorem}
  The domain recursion by Dilkas and Belle~\shortcite{DBLP:conf/kr/DilkasB23}
  can simulate the domain recursion by Van den
  Broeck~\shortcite{DBLP:conf/nips/Broeck11}. This proves that Crane can (still)
  handle \FOtwo.
\end{theorem}
...
\end{itemize}
% ** bijections max time: 123.870
% *** Crane-BFS scales up to 10k in 34 s
% **** answer = 10k! = a number with 35660 digits
% ** functions max time: 78.330
% *** both Crane-BFS and Crane-Greedy scale up to 300k in 41 s
% **** the count of this is 300k^300k is a number with over 1 million digits
