...
\usepackage[backgroundcolor=lightgray]{todonotes} % TODO: remove
\usepackage{listings}
...
constructs that describe these sequences. Recently, WFOMC algorithms have also
...
al.~\shortcite{DBLP:conf/pods/BeameBGS15} showed that there exists an unliftable
...
The recently-proposed \textsc{Crane} algorithm, while able to handle formulas
beyond the reach of \textsc{ForcLift}, was somewhat incomplete in that it could
only construct functions definitions, which would then need to be evaluated to
compute the WFOMC.\@ Moreover, recursive functions were presented without base
cases. In this work, we present \Cranetwo{}---an extension of \textsc{Crane}
that fixes these two weaknesses.

\todo[inline,caption={}]{
  \begin{itemize}
  \item Finish the paragraph above.
  \item mention that \textsc{L2C}~\cite{DBLP:conf/kr/KazemiP16} compiles to C++
  \end{itemize}
}
...
\Cref{fig:overview} outlines the workflow of the new algorithm \Cranetwo{}. In
...
\end{enumerate*}
Then, \cref{sec:smoothing} describes post-processing techniques for FCGs and the
formulas from \cref{step2} above necessary to preserve the correct model count.
Next, \cref{sec:cpp} explains how function definitions that encode a solution to
a WFOMC problem are then compiled to a C++ program. Finally,
\cref{sec:experiments} reports on two experiments that compare \Cranetwo{} with
other state-of-the-art WFOMC algorithms.
...
\begin{example}[Counting functions]\label{example:functions}
  ...
  observed by Dilkas and Belle~\shortcite{DBLP:conf/kr/DilkasB23}, such a
...
Van den Broeck et al.~\shortcite{DBLP:conf/ijcai/BroeckTMDR11} generalise
...
den Broeck et al.~\shortcite{DBLP:conf/ijcai/BroeckTMDR11}.
\Cref{sec:smoothingbase} describes the role smoothing plays in the
...
Smoothing is a two-step process. First, positive unit clauses (denoting sets of
ground atoms that are accounted for in the FCG) are propagated `upwards', i.e.,
in the opposite direction of FCG arcs. Then, at nodes of certain types, missing
atoms are detected and additional nodes are added to account for them. In this
section, we:
\begin{enumerate*}[label=(\roman*)]
  \item describe the relevant node types from previous work,
  \item show how smoothing ought to work for these node types, and
  \item illustrate how the new smoothing techniques work on two example FCGs.
\end{enumerate*}

Before describing the proposed changes to smoothing, we briefly review the
compilation rules (and their corresponding node types) introduced by Dilkas and
Belle~\shortcite{DBLP:conf/kr/DilkasB23} while referring to the aforementioned
paper for precise definitions. \emph{Domain recursion} selects a domain
$\Delta$, introduces a new constant $c \in \Delta$, and, for each variable $x$
quantified over $\Delta$, considers two possibilities: $x = c$ or $x \ne c$,
modifying the formula accordingly. We denote the resulting node as
$\DR(c \in \Delta)$. \emph{Constraint removal} applies to formulas such that,
for some domain $\Delta$, each variable $x$ quantified over $\Delta$ is followed
by the inequality constraint $x \ne c$. In other words, the relevant clauses can
be rewritten to begin with $\forall x \in \Delta\text{.
}x \ne c \Rightarrow \ldots$ For such formulas, constraint removal replaces
$\Delta$ with a new domain $\Delta'$ and removes the $x \ne c$ constraints. We
denote the resulting node as $\CR(\Delta' \gets \Delta \setminus \{\,c\,\})$.
Finally, \emph{caching} detects when the input formula $\phi$ is equal to a
previously-encountered formula $\psi$ except for having different domains. We
denote the resulting node as $\Reff(\sigma)$, where $\sigma$ is the substitution
...
\begin{example}[An FCG that needs smoothing]
  \Cref{fig:smoothing2} shows an FCG with a domain recursion node
...
\todo[inline]{Introduce greedy search and breadth-first search (BFS), together
  with \Cranegreedy{} and \Cranebfs{}. The knowledge compilation part of both
...
including 35. \Cref{fig:plot} shows total runtime values as a measure of the
