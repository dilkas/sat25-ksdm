...
\todo[inline]{Reorder the Logic subsection to cover \FO{} before comparing it to
  other logics (and clarify the differences between the input format and the
  internal format)}
...
For any variable $v$, let $\Dom(v)$ denote the domain over which $v$ is
quantified. For any clause or literal $l$, let $\Vars(l)$ denote the set of
variables in $l$, and, similarly, let
$\Doms(l) \coloneqq \{\, \Dom(v) \mid v \in \Vars(l) \,\}$.
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item the definition of a function is a strong assumption
    \item the notation for a function call is now outdated
  \end{itemize}
}

\begin{itemize}
  \item We write $\expr{}$ for an arbitrary algebraic expression.
  \item A few terms have different meanings in the context of algebra as
        compared to logic:
        \begin{itemize}
          \item a \emph{variable} is either:
                \begin{itemize}
                  \item a parameter of a function or
                  \item a variable introduced by a summation, e.g., $i$ in
                        $\sum_{i=1}^{n} \expr$.
                \end{itemize}
          \item A \emph{constant} is a non-negative integer.
        \end{itemize}
\end{itemize}
...
\begin{algorithm}[t]
  ...
  $\mathcal{E} \gets \Simplify{$\mathcal{E}$}$\;
...
\todo[inline]{In \cref{alg:main}, some kind of translation also needs to take
  place, right?}

We write \Crane for the first-order knowledge compilation algorithm of the
original \textsc{Crane}~\cite{DBLP:conf/kr/DilkasB23} that compiles a formula
$\phi$ into:
\begin{itemize}
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item \textsc{Crane}, \Cranetwo{}, or \Crane in the paragraph below
  \end{itemize}
}

See \cref{alg:main}. The evaluation of base cases is done by simplifying the
clauses and then using \textsc{Crane} to find the base cases. A particular
domain is selected, and the clauses are simplified. Then, \textsc{Crane} is
called on those clauses to evaluate the base cases. After that, we change the
function names and variable to make it consistent with the previous domain to
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item introduce the Iverson bracket (it's only used in this subsection)
    \item \Cref{alg1} is now fixed and supersedes the description below (and the
          code!)
    \item NOTE: exactly one of the if and else-if branches always applies
  \end{itemize}
}
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item later check if I'm still using the syntax for $\Vars$, $\Dom$, etc.
    \item align the last line to be a bit neater
    \item \Cref{alg2} now supports any base case domain sizes
    \item NOTE: if $C' = \emptyset$, then the clause becomes irrelevant and
          doesn't need smoothing, so can be skipped
  \end{itemize}
}
...
\todo[inline,caption={}]{ Turn these notes into a coherent description (that
  doesn't mention \textsc{Crane}):
  \begin{itemize}
    \item Each base case identified by \FindBaseCases can be interpreted as
          assuming some domain...
    \item \Propagate (\cref{alg2}) to find the transformed formula corresponding
          to each base case identified by \FindBaseCases and call \textsc{Crane}
          on the formula to obtain the required base cases.
...
\todo[inline]{Maybe prove consistency: if two base case overlap, their numerical
  answers also overlap.}
...
\todo[inline]{Can I use an example based on permutations or something similar?}
