...
% TODO: No proper runtime consideration (other than the termination). (address
% this criticism in some way)
...
% TODO: Updates to the algorithm
% 1) replace x_i-c_i on Line 7 of Algorithm 2 with a generalisation of x_i that can be an arbitrary summation/subtraction of domain sizes
% 2) a function call with a constant should always mean that that function should have a base case, not just when it's in the same function

% TODO: consider restructuring the paper to make it easier to understand

\section{Introduction}
% TODO: improvements to the introduction (must):
% * make it extra clear what the gap is that this work is trying to fill
% * mention Crane earlier and describe its importance
% * the last two paragraphs should make more of a BOOM (e.g., include the over 9k claim)
% * (here and elsewhere) clarify the distinction between the weighted and unweighted variants of the problem in the final version of the paper. We focus on the unweighted variant in our implementation and experiments, however, Crane2 can handle WFOMC instances as well simply by switching the main data type from integers to rational numbers. Maybe add some weighted experiments as well.
% * (must) more related work, situating my contributions in the broader context
% * (concerning related work) the topic is intimately related to "graded logics", largely investigated in DL, mu-calculus and temporal logics
...
% TODO: FCGs were never defined. Maybe avoid referring to them altogether?
% Smoothing nodes are also undefined.

% 5. What are the key components of my approach and results? Also include any
% specific limitations.

% TODO: first sentence: elaborate a bit more for a general audience
...
% TODO: We called the previous version of Crane "incomplete" because it cannot
% be used as a black box that takes Markov logic networks (or weighted formulas
% in some other format) and outputs numbers (i.e., model counts or
% probabilities).
...
WFOMC problem are compiled into C++ programs.\footnote{The possibility of
...
% TODO: explain that "Skolemization" here is NOT standard Skolemization of
% first-order logic, which would replace existential quantifiers by function
% terms.

% TODO: reorder the definitions in the opposite direction
...
% TODO: mention the Unique Name Assumption and that, hence, s = t holds iff s
% and t are the same constant.
...
% TODO:
% 1. Rename \UFO{} to what that new paper uses
% 2. Use FastWFOMC.jl instead, removing the mention of 'privately-obtained'
% 3. use symbols instead of numbers for the footnotes
...
% TODO: the discussion about sorts isn't featured in the rest of the paper
...
% TODO: The distinctness of domains is important in two ways. First, in terms of
% expressiveness, writing clause such as "\forall x, P(x, x)" is valid if
% predicate P is defined over two copies of the same domain and invalid
% otherwise. Second, having more distinct domains makes the problem more
% decomposable for the knowledge compilation algorithm. With more distinct
% domains, it can make assumptions or deductions about, e.g., the first domain
% of predicate P without worrying how (or if) they apply to the second domain.
...
% TODO: “Weights in Q” undefined, C2 undefined (and also far from well explained
% before), endofunction undefined (also endomorphisms missing), weight function
% undefined, weighted model count imprecise (above it’s called weighted
% first-order model count/wfmoc),
...
\begin{example}[Counting functions]\label{example:functions}
  ...
  % TODO: Yes, as mentioned at the end of Section 2.1, clauses c and d written
  % on separate lines are to be interpreted as the conjunction of c and d (maybe
...
% TODO: As mentioned in the caption of Figure 1, "Compile" refers to the
% knowledge compilation algorithm of Dilkas and Belle (2023). Its three return
% values are explained at the beginning of page 4. We do not elaborate on the
% details of "Simplify" as it primarily consists of elementary algebraic
% simplifications such as "1x = x" and "0x = 0". However, we highlight one
% non-trivial aspect of "Simplify" right before Section 3.1. We will improve the
% structure of Section 3 to make this information more readily available. Maybe
% write Crane instead of Compile?

% TODO: It would not be entirely accurate to call the algorithms presented in
% this work a "post-processing step" because Algorithm 1 is recursive. In other
% words, this "post-processing step" to first-order knowledge compilation can
% lead to more compilation that then updates/completes the original set of
% equations.
\begin{algorithm}[t]
...
\end{algorithm}
% TODO: >> Alg 1, line 5: Why is this not an infinite loop? On line 5 of
% Algorithm 1, the loop iterates over the indices of \mathbf{x}. The arity of a
% function is always a finite number, typically equal to 1 or 2.
...
% TODO: F missing (also not explained that it’s not needed below). E is not explained other than that we take the example from X. Would not be a problem if it is motivated differently and the focus is on post-processing (defining inputs properly) and not on the entire algorithm.
...
% TODO: maybe omit the word 'iverson'...
...
% TODO: To be precise, line 7 of Algorithm 3 checks whether smoothing is
% necessary, and lines 8-9 execute it. If the condition on line 7 is not
% satisfied, the clause is not smoothed but rather completely omitted.
...
example below.
% TODO: In this context, "not unique" means that there are many different ways
% to adjust/rewrite the formula to achieve the goal of smoothing. This section
% (and Algorithm 3) presents one such approach.
...
% TODO: A WFOMC instance comprises of a formula in first-order logic, a pair of
% weights for each predicate, and a domain size (i.e., a non-negative integer)
% for each domain. We provide the full details of all of these, including how
% the formulas ought to be rewritten to fit the format of each WFOMC algorithm.

% TODO: must rerun on an HPC and include more instances

% TODO: "hard to follow", pointing to the DPMC paper as a good example
...
% TODO: maybe instead of a table I should have an experiment where:
% 1) Crane scales exponentially (with a base of sqrt(2))
% 2) other algorithm scale linearly
% 3) Then I can just draw the usual cumulative plot
% 4) Stop each algorithm when they reach the timeout value
...
% TODO: I'm claiming that C++ code generation is not the main contribution, but
% the experiments are mainly about it...
...
% TODO: position some of the experiments as 'practical'
...
% TODO:
% must clean pdf metadata before submitting
% make sure that it's not automatically deducible that I'm the author of Crane
% no type 3 fonts (including in the PDFs of figures)
% no overfull boxes (might be the figure)
% no embedded links (but URLs are fine as long as they're not clickable?)
% must not use 'input' command. instead, compile to a pdf
% figures should appear on the page where they are discussed or on the next page
% colors in figures must be cmyk, not rgb
% references must not have missing fields
