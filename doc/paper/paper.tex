...
\todo[inline,caption={}]{
  ...
  \begin{itemize}
    \item Perhaps I can list the `authors contributed equally' statement as part
          of Funding. Check K's recent papers.
...
\Cref{sec:propagating} introduces the \Propagate procedure that takes a formula
...
second benchmark is a variant of the well-known `Friends and Smokers' (hitherto
referred to as `friends') Markov logic
network~\cite{DBLP:conf/aaai/SinglaD08,DBLP:conf/uai/BroeckCD12}. In \Ctwo{},
\FO{}, and \UFO{}, this problem can be formulated as
\[
...
\todo[inline]{For each curve, check which polynomial or exponential model best
  fits the data to expand this section a bit (without plotting them, but
...
\todo[inline]{Incorporate into the text below: It is worth remarking that
  \textsc{ForcLift} does not support arbitrary precision, and returns error for
  cases that requires arbitrary precision reasoning. }
...
\todo[inline,caption={}]{
  \begin{itemize}
    ...
    \item Delete references to this appendix (except for one place, probably in
          \cref{sec:experiments})
    \item Make sure the rest of the paper doesn't refer to these logics (or
          describe examples in them)
    \item Move the URLs to the main text
