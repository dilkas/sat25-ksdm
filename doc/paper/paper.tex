...
\todo[inline,caption={}]{
  ...
  \begin{itemize}
    \item Finish adding DOIs or URLs to (non-book) references (and reformatting
          indentation) (and check reference formatting)
    \item Sentence vs formula: be consistent and not confusing
    \item 15 pages excluding references
    \item Go through the rest of formatting instructions (in GTD)
  \end{itemize}
}
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item We would like to clarify that the main contribution of this work
          ...
          identifying the base cases and their corresponding formulas,
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item Don't assume that the reader knows FOMC and FOMC algorithms
    \item It would be nice to give the necessary introduction to FOKC, or
          \textsc{Crane} (an intuitive explanation early in the paper)
    \item The principal motivation of a knowledge compilation-based approach to
          FOMC comes from its ability to find polynomial-time solutions, where
          the polynomial has a lower degree compared to, e.g.,
          \textsc{FastWFOMC}~\cite{DBLP:conf/kr/DilkasB23}. The ability of the
          algorithm to mathematically describe recursive solutions is also
          noteworthy as the only other available alternative is to guess
          them~\cite{DBLP:conf/ilp/BarvinekB0ZK21}.
  \end{itemize}
}
...
\todo[inline]{Make sure to revisit this in the future}

\subsection{FOMC Algorithms and Their Logics}\label{sec:threelogics}

\todo[inline]{Shorten the part about the three logics}
...
\todo[inline]{Even though we are evaluating the algorithms on FOMC benchmarks,
  for sentences with existential quantifiers, computing FOMC using \Cranetwo{}
  requires the use of WFOMC. For such sentences, Skolemization introduces
...
is $f(\mathbf{x}) = \expr{}$, where $f(\mathbf{x})$ is a signature.
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item The algorithm’s recursive nature and its interaction with
          \textsc{Crane} could be explained more clearly
    \item More emphasis on the motivation for each part of \Cranetwo{} (e.g.,
          how \FindBaseCases addresses key issues in \textsc{Crane}) would add
          depth to the contribution
    \item this section should be expanded by 1 page (``a clear discussion of the
...
\begin{figure*}[t]
  ...
  \caption[]{The outline of using \Cranetwo{} to compute the model count of a
    ...
    procedure makes use of the FOKC algorithm of \textsc{Crane}, algebraic
...
compiling equations into C++ code, greatly expanding upon the range of formulas
that could previously be handled by similar
approaches~\cite{DBLP:conf/kr/KazemiP16}.

\todo[inline]{Somewhere: Recall that each equation is either the definition (did
  I define what a definition is?) of a function or the definition of a base case
  of a function. (I could put this in the preliminaries as definitions and then
...
\todo[inline]{Move this example right after the definition of WFOMC (or near the
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item Experiments could be expanded by 1 page (``a more thorough and
          independent practical assessment'')
    \item The difference between \Cranegreedy{} and \Cranebfs{} is in how the
          algorithm chooses which compilation rule to apply to a formula. The
          former uses greedy search: there is a list of rules, and the first
          applicable rule is the one that gets used, disregarding all the
          others. The latter uses a combination of greedy and breadth-first
          search (BFS). That is, each compilation rule is identified as either
          greedy or non-greedy. Greedy rules are applied as soon as possible at
          any stage of the compilation process. BFS is executed over all
          applicable non-greedy rules, identifying the solution that can be
          constructed using the smallest number of such rules.
  \end{itemize}
}
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item More benchmarks from NOT my work
    \item Make sure it's clear what the `raw' instances are
    \item Don't emphasise that the benchmarks come from my previous work
  \end{itemize}
}
...
\todo[inline]{Shrink/restructure to fit into the margins}
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item On the `friends' and `functions' benchmarks, \textsc{ForcLift} runs
          until the model count exceeds $2^{31}-1$.
    \item We are not aware of any formulas on which \Cranetwo{} scales worse
          compared to either \textsc{ForcLift} or \textsc{FastWFOMC}. The one
          advantage that \textsc{FastWFOMC} has over \Cranetwo{} is its support
          for counting quantifiers.
    \item Regarding programming languages and accuracy, we verified that the
          answers match for smaller domain sizes. Also, although written in
          different languages, both \Cranetwo{} and \textsc{FastWFOMC} use the
