...
\todo[inline,caption={}]{
  ...
  \begin{itemize}
    \item Finish adding DOIs or URLs to (non-book) references (and reformatting
          indentation) (and check reference formatting) (is the part about
          non-book references correct? Check previous proceedings) (maybe one
          more exception (graded logics)?) (I might be done)
    ...
    \item Make sure all the references (e.g., to examples are still valid)
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item If I need more space elsewhere, shortening the preliminaries to two
          pages (perhaps skipping Section 2.2 altogether?)
    \item Adjust the introductory paragraph below to the new structure (with
          more subsections)
  \end{itemize}
}
...
% Additionally, our formulation of the definition of WFOMC is
% somewhat complicated by the need to support any number of domains.
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item Reorganise this section (e.g., where do I put the description of
          Simplify?), bringing some TODOs from above.
...
\Cref{example:bijections} below provides more detail.
...
\begin{example}\label{example:bijectionstwo}
  ...
  and its initial solution described in \cref{example:bijectionstwo}. On
  \autoref{line:second}, \Simplify transforms
  $g(l, m) = \sum_{k=0}^{m}[0 \le k \le 1]\binom{m}{k}g(l-1, m-k)$ into
  $g(l, m) = g(l-1, m) + mg(l-1, m-1)$. Then \FindBaseCases identifies two base
  cases: $g(0, m)$ and $g(l, 0)$. In both cases, \CompileWithBaseCases recurses
  on the formula $\mathcal{F}(g)$ simplified by assuming that one of the domains
  is empty. In the first case, we recurse on the formula
  ...
  \Propagate{$\psi$, $\Gamma$, $0$} returns an empty formula, resulting in
  $g(l, 0) = 1$.

  While these base cases overlap when $l = m = 0$, they remain consistent since
  $0^{0} = 1$. Generally, let $\phi$ be a formula with two domains $\Gamma$ and
  $\Delta$, and let $n, m \in \mathbb{N}_{0}$. Then the FOMC of
  \Propagate{$\phi$, $\Delta$, $n$} assuming $|\Gamma| = m$ is the same as the
...
\begin{example}
  Consider the recursive function $g$ from \cref{example:bijectionstwo}.
...
\begin{algorithm}[t]
  \caption{A sketch of the C++ program for the equations in \cref{example:bijectionstwo}, particularly highlighting the recursive definition of function $g$.}\label{alg:cpp}
...
\todo[inline,caption={}]{
  \begin{itemize}
    ...
    \item Integrate the example below into the text, possibly moving irrelevant
          details (such as the formulas in other logics) to supplementary
          material
  \end{itemize}
}
...
in \cref{example:bijections}. The equivalent formula in \Ctwo{} is
\[
