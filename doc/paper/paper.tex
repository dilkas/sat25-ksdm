...
\todo[inline]{Adjust the introduction to the new structure (with more
  subsections)}
...
For such sentences, Skolemization (described in \cref{sec:crane}) introduces
predicates with non-unary weights that must be accounted for to compute the
correct model count. Additionally, our formulation of the definition of WFOMC is
...
\begin{example}[Counting bijections]\label{example:overall}
  Let $\phi$ be the following formula (previously examined by Dilkas and
  ...
  $\sigma(\Delta) \coloneqq \{\,a, b\,\}$. Then $\phi$ has two models:
  \[
...
\todo[inline]{It would be nice to give the necessary introduction to FOKC, or
