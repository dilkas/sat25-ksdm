...
(symmetric) weighted variant of this problem, known as WFOMC, seeks to compute
the total weight of the models~\cite{DBLP:conf/ijcai/BroeckTMDR11}. WFOMC
relates to its propositional predecessor weighted model
counting~\cite{DBLP:journals/ai/ChaviraD08} and, more conceptually, to other
attempts to unify logic and probability such as probabilistic
logic~\cite{DBLP:journals/ai/Nilsson86}, fuzzy
logic~\cite{novak2012mathematical}, and graded logics~\cite{vsaletic2024graded}.
Moreover, WFOMC is a key approach to \emph{lifted (probabilistic) inference},
which aims to compute probabilities more efficiently by leveraging symmetries
inherent in the problem description~\cite{DBLP:conf/ecai/Kersting12}.
...
increasing sizes, the FOMC of a formula can be seen as an integer sequence. FOMC
algorithms have been utilised for discovering new
...
recursive structures that describe these sequences. Additionally, FOMC
...
There are various FOMC algorithms with different underlying principles. Perhaps
the most prominent class of FOMC algorithms is based on \emph{first-order
...
FOMC for any combination of domain sizes. Algorithms in this class include
...
circuits, while the latter compiles them first to graphs and then to (algebraic)
equations. Another FOMC algorithm,
...
incomplete in that it could only construct function definitions, leaving users
...
base cases, adding further complexity to the user.

In this work, we present \Cranetwo{}, the breakthrough that not only addresses
these gaps but also pushes the boundaries of scalability to unprecedented
levels. With \Cranetwo{}, we eliminate the manual steps, providing a fully
automated FOMC algorithm. Moreover, \Cranetwo{} can scale to domain sizes over
\num{500000} times larger than the previous state of the art, redefining what is
possible in the field of first-order model counting.
...
\cref{sec:cpp} elucidates how function definitions encoding a solution to a FOMC
problem are compiled into C++ programs. Note that a solution to FOMC that uses
compilation to C++ has been considered before~\cite{DBLP:conf/kr/KazemiP16},
however, the extent of formulas that could be handled was limited. Finally,
\cref{sec:experiments} presents experiments results comparing \Cranetwo{} with
other state-of-the-art FOMC algorithms.

\section{Preliminaries}

In \cref{sec:logic}, we provide a summary of the basic principles of first-order
logic. Then, in \cref{sec:threelogics}, we formally define (W)FOMC and discuss
the distinctions between three variations of first-order logic that are utilised
for FOMC.\@ Finally, in \cref{sec:algebra}, we introduce the terminology used
to describe the output of the original \textsc{Crane} algorithm, i.e., functions
...
While the focus of this work is on FOMC, we still define the weighted variant of
the problem as Skolemization relies on weights even for unweighted FOMC.
...
Before describing the main contribution of this work, let us review the key
aspects of FOKC as realised by \textsc{Crane}. The input formula is compiled
into:
\begin{enumerate*}[label=(\roman*)]
...
represents the solution to the FOMC problem. To compute the FOMC of the formula
for particular domain sizes, $f$ must be evaluated with those domain sizes as
arguments. $\mathcal{D}$ records this correspondence between function arguments
and domains.
...
some algebraic simplification (explained at the end of this section),
...
defined. For each base case $f(\mathbf{x})$, we retrieve the logical formula
...
\begin{example}[Counting bijections]\label{example:overall}
  ...
  \end{align*}
  where $\Delta^{\top}$ is a new domain introduced by \Compile. (We omit the
  definition of $\mathcal{F}$ as the formulas can get a bit verbose.) Then
...
In the rest of this section, we show that the base cases identified by
\FindBaseCases are sufficient for the algorithm to terminate. Note that
characterising the fine-grained complexity of the solutions found by \Cranetwo{}
or other FOMC algorithms is an emerging area of research partially addressed by
\citeauthor{DBLP:conf/kr/DilkasB23}~\shortcite{DBLP:conf/kr/DilkasB23} and
\citeauthor{tóth2024complexityweightedfirstordermodel}~\shortcite{tóth2024complexityweightedfirstordermodel}.
...
\begin{example}\label{example:basecasesmoothing}
  ...
  correct FOMC should be $2^{|\Gamma|^2 - |\Gamma'|^2}$.
...
second is a variant of the well-known `Friends and Smokers' Markov logic
