...
\begin{frame}[fragile]{Finding (a Sufficient Set of) Base Cases}
  \begin{block}{Outline}
    \begin{enumerate}
      \item For every function call:
            \begin{enumerate}
              \item For every argument of the form
                    \structure{$\textit{var} - \textit{const}$}:
                    \begin{enumerate}
                      \item Replace the signature parameter with
                            \structure{$0$}, \structure{$1$}, \dots,
                            \structure{$\textit{const} - 1$}
                    \end{enumerate}
              \item For every argument of the form \structure{$\textit{const}$}:
                    \begin{enumerate}
  ...
  \end{block}
  \begin{example}
    The signature of \structure{$g$} is \structure{$g(l, m)$}.
...
% TODO: propagate + maybe smoothing
\begin{frame}{From a Base Case to a Sentence}
