...
% TODO: 5 slides explaining the solution (maybe including the theorems)
% 0. Preprocess
...
% TODO: (here and on the next slide) mark which steps will be explained in more detail
...
\begin{frame}[fragile]{Knowledge Compilation Workflow (2/2)}
  ...
  \begin{tikzcd}
    g(0, m) \arrow[d, gray, visible on=<2->] & \phantom{-} \arrow[d, phantom, visible on=<2->, "\text{\structure{4.1.} Construct the corr.\ sentence}"] & g(l, 0) \arrow[d, gray, visible on=<2->] \\
    |[visible on=<2->]| \forall x \in \Gamma\text{. }S(x) \lor \neg S(x) \arrow[d, gray, visible on=<3->] & \phantom{-} \arrow[d, phantom, visible on=<3->, "\text{\structure{4.2.} Recurse}"] & |[visible on=<2->]| \top \arrow[d, gray, visible on=<3->] \\
